...
int VisionClient::runVision(
    ...
    // Initialize VisionConfig, VisionA, and FrameProcessor
    VisionConfig cfg = VisionConfig::fromYaml("../../assets/vision/config/vision.yml");
    VisionA vision(cfg);
    FrameProcessor fp;
    ...
    if (output_state_parent_path.empty()) {
        ...
        if (error_code_) {
            std::cerr << "[VisionClient] Failed to create output state directory: " << output_state_parent_path.string() << " : " << error_code_.message() << "\n"
                      << "       Hint: check if the path is valid. By default the path for saving output states is ../../out/000000.jsonl \n";
            return 1;
        }
    }
    ...
    std::cout << "[VisionClient] Latest frame snapshot: " << output_state_parent_path.string() + "/last_frame.jsonl" << "\n";
    return 0;
}
