...
void AdminWindow::initWsClient() {
    ...
    connect(ws_, &QWebSocket::textMessageReceived, this, [this](const QString& msg){
        ...
        // else if (type == "seat_update") {
        //     // （可选）先更新统计
        //     const auto stats = obj.value("stats").toObject();
        //     // updateTopStats(stats); // 如果你有统计卡片
        ...
        //         const QString id    = o.value("id").toString();
        //         const int     code  = mapStateTextToCode(o.value("state").toString());
        //         const QString since = unifyTsToLocalIso(o.value("last_update").toString());
        //         // 更新宫格/表格（名字按你的项目来，这里用你现有的函数名举例）
        //         setSeatCell(id, code, since);
        //         if (seatTable_) {
        //             upsertRow(seatTable_, id, code, since, nowUtc);
        //         }
        //     }
        // }
        // 管理员端接收 WS 消息
            ...
            for (const auto& v : seats) {
                ...
                static std::unordered_map<QString, int> anomaly_streak;  // 用于存储每个座位的Anomaly连击次数

                int streak = anomaly_streak[id];
                const QString state = o.value("state").toString();

                if (state == "Anomaly") {
                    anomaly_streak[id] = streak + 1; // Anomaly 连击数累加
                } else {
                    anomaly_streak[id] = 0; // 非 Anomaly 重置连击数
                }

                // 判断是否占座
                bool isOccupied = (state == "Seated") || (anomaly_streak[id] >= 6); // 6次连续Anomaly为占座

                int st = 0; // 默认空闲
                if (isOccupied) st = 2;  // 占座
                else if (state == "Seated") st = 1;  // 有人
                else st = 0;  // 没人

                const QString since = unifyTsToLocalIso(o.value("last_update").toString());
                setSeatCell(id, st, since); // 更新 UI
            }
        }
    ...
    connect(ws_, &QWebSocket::textMessageReceived, this, [this](const QString& msg){
        ...
        // 管理员端接收 seat_update 消息
            ...
            for (const auto& v : seats) {
                ...
                // 直接从消息中读取 occupied 字段
                // occupied=true 仅当连续6次及以上Anomaly
                bool occupied = o.value("occupied").toBool();

                // 管理员端只需要两种状态：占座(2)和不占座(0)
                int code = occupied ? 2 : 0;
                ...
                // 调试输出
                qDebug() << "[Admin] Seat:" << id
                         << "raw_state:" << o.value("state").toString()
                         << "anomaly_streak:" << o.value("anomaly_streak").toInt()
                         << "occupied:" << occupied
                         << "display_code:" << code;
            }
        }
