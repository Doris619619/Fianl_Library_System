...
StudentWindow::StudentWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    pages->addWidget(buildBookSearchPage()); // 放在最后一个
    ...
    // 3) 信号槽：点击切换
    connect(btnBook, &QPushButton::clicked, this, [this]{
        if (!bookPage) return;
        int idx = pages->indexOf(bookPage);
        if (idx >= 0) pages->setCurrentIndex(idx);
...
QWidget* StudentWindow::buildBookSearchPage() {
    ...
    // 首次进入时尝试加载 Input/books.txt
    if (!bookEngine.ready()) {
        const QString runtimePath = QCoreApplication::applicationDirPath()
                                    + "/Input/books.txt"; // 由 CMake 拷贝过去
        QString err;
        bookEngine.loadFromFile(runtimePath, &err); // 失败不弹错，按需在搜索时提示
    }
...
void StudentWindow::onSearchBooks() {
    ...
    if (!bookEngine.ready()) {
        const QString runtimePath = QCoreApplication::applicationDirPath()
                                    + "/Input/books.txt";
        QString err;
        if (!bookEngine.loadFromFile(runtimePath, &err)) {
            CardDialog(u8"读取失败", u8"无法读取 Input/books.txt，请确认已拷贝到运行目录。", this).exec();
            return;
        }
    }
...
void StudentWindow::gotoLive() {
    // 假设“座位实况”被 addWidget 到页索引 4（第 5 页）
    // 如果你的页数不同，也无所谓；你也可以在 addWidget 后立即保存索引到成员变量再用
    // 这里用最稳妥的方法：直接查找 livePage 在 pages 里的索引
    if (!livePage) return;
    auto pages = findChild<QStackedWidget*>();
    if (!pages) return;
    int idx = pages->indexOf(livePage);
    if (idx >= 0) pages->setCurrentIndex(idx);
